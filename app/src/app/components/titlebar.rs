    ...
    fn with_frame(ctx: &mut Self::Context, ui: &mut Ui, frame: &mut Frame) {
        ...
        ui.horizontal(|ui| {
            ui.with_layout(Layout::right_to_left(), |ui| {
                ...
                ui.add_space(300.);
                let title = eframe::egui::Label::new("fsLister");
                ui.add(title);
        ...
        let (response, painter) = ui.allocate_painter((ui.available_width(), 2.).into(), Sense::hover());
        if response.hovered() {
            frame.drag_window();
        }

        let rect = response.rect;
        let stroke = Stroke::new(2., Color32::from_hex_panic("#78909c"));
